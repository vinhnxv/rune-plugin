...
> Reasoning principles for Elden Lords and Tarnished teammates in multi-agent workflows.

Applies Sequential Thinking principles to multi-agent workflows at both the **lead orchestration** and **Tarnished reasoning** levels. All guidance here is prompt engineering — no new tools or infrastructure required.
...
| Lead (monitoring) | "Tarnished timed out — revise inscription expectations, mark as partial" |
| Tarnished (self-review) | Re-read output file, fix weak Rune Traces, downgrade or delete hallucinated findings |

**Key constraint**: Tarnished get ONE revision pass to prevent infinite loops. Lead can revise multiple times during aggregation.
...
| Level | Application |
|-------|-------------|
| Lead (aggregation) | When 2+ Tarnished flag same code differently, branch to explore both interpretations before choosing |
| Tarnished | **Not applicable** — Tarnished work in isolation and don't benefit from branching |
...
| Tarnished | Already externalized via output files. Self-review reads the written file, not memory |
...
|---------|---------|-------------|
| 1 | Count and Estimate | How many Tarnished? What's the token budget? |
| 2 | Choose Strategy | Task-only, Agent Teams, or hybrid? |
| 3 | Output Directory + Inscription | Where do Tarnished write? Generate inscription.json |
| 4 | Verify Protocol Injection | Does each Tarnished prompt include Glyph Budget + Seal format? |
...
Use structured reasoning when Tarnished produce conflicting findings:
...
Step 2: Verify evidence
  ...
  - Which Tarnished's Rune Trace matches reality?
...
## Tarnished Self-Review Protocol

**Embedded inline in each Tarnished's prompt — no external file reads required.**
...
|---------------|-------------------------|---------------------------|
| Tarnished count → strategy | | Use table in context-weaving |
| File pattern → Tarnished mapping | | Use conditional Tarnished table in Rune Gaze |
| Confidence → spot-check count | | Use matrix in inscription-protocol |
| Merge redundant Tarnished? | When trade-offs are unclear | When Tarnished clearly overlap (same files, same checks) |
| Conflicting Tarnished findings | Always (Tarnished disagree on same code) | |
| Partial failure recovery | When impact is uncertain | When failed Tarnished was clearly non-critical |
| Scope revision mid-work | When new patterns change assumptions | When change is additive (just add a conditional Tarnished) |
...
The self-review checkpoint does NOT depend on Sequential Thinking MCP. It is embedded directly in Tarnished prompts.
...
| Self-review (Tarnished) | ~10 lines of prompt text per template |
| Total overhead per review session | ~40 lines across up to 5 Tarnished templates |
...
| Conflicting Tarnished findings | File pattern → Tarnished mapping |
...
- Tarnished prompts: `../../roundtable-circle/references/tarnished-prompts/` → Self-Review Checkpoint
