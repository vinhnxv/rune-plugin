...
> 7-section prompt template for Tarnished teammates with context rot prevention.

## The Problem: Context Rot in Teammates

When a Tarnished receives a 200k token context window filled with code files to review, the model's attention on critical instructions (evidence rules, output format, Truthbinding protocol) degrades. This is the Lost-in-Middle effect — content in the middle of long contexts receives less attention.
...
Every Tarnished prompt MUST follow this structure:
...
6. Send Seal to the Elden Lord (max 50 words)

## PERSPECTIVES
[The review perspectives this Tarnished embodies]
...
SendMessage to the Elden Lord: "Seal: {role} complete. Path: {path}. Findings: N P1, N P2, N P3."
...
Limit files per Tarnished to prevent cognitive overload:

| Tarnished | Max Files | File Types |
|-----------|----------|------------|
...
If a Tarnished is assigned more files than its budget, split into multiple tasks.
...
Each Tarnished performs one self-review pass before sending the Seal. Document in output:
...
Tarnished CAN ask questions to the Elden Lord via SendMessage. This is non-blocking — the Tarnished continues reviewing while waiting for a response.
...
| **Tier 2: Lead Clarification** | Blocking ambiguity | SendMessage to the Elden Lord, continue with non-blocked files |
| **Tier 3: Human Escalation** | Critical unknown | Document in output file for human review |

**Rules:**
- Max 1 Tier 2 request per Tarnished per session
