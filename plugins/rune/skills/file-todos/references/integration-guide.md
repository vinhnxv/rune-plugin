...
File-todos are always generated by all Rune workflows. No talisman opt-in is required. Use `--todos=false` on any command to disable for a single invocation.
...
- `--todos-dir` with `--todos=false`: flag is accepted, directory is NOT created, no error raised
...
**Config**: Always active. Disable per-invocation with `--todos=false`.
...
**Escape hatch**: Pass `--todos=false` to strive to skip file-todo generation and use only the per-worker log protocol (`tmp/work/{timestamp}/todos/{worker-name}.md`).
