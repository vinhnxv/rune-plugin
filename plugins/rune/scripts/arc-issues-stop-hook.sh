...
At the BEGINNING of your response, run these GitHub cleanup steps for completed issues:
  GH_PROMPT_DISABLED=1 gh issue list --json number,labels --jq '.[] | select(.labels[].name == \"rune:in-progress\") | .number' | while read -r num; do
...
# Steps: post result comment on COMPLETED issue + update rune:in-progress → rune:done label.
GH_STATUS_STEPS=""
...
Step B — Update labels (remove rune:in-progress, add rune:done):
  GH_PROMPT_DISABLED=1 gh issue edit ${_CURRENT_ISSUE_NUM} --remove-label \"rune:in-progress\" --add-label \"rune:done\" 2>/dev/null || true
